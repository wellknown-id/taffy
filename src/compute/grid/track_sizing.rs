...
pub(super) fn track_sizing_algorithm<Tree: LayoutPartialTree>(
    ...
    // For the purpose of expansion steps ("Maximise Tracks", "Expand Flexible Tracks" and "Stretch auto Tracks"),
    // we only want to expand into space generated by the grid container's size (as defined by either it's preferred
    // size style or by it's parent node through something like stretch alignment), not just any available space.
    // To do this we map definite available space to AvailableSpace::MaxContent in the case that inner_node_size is None
    // unless the available grid space is explicitly constrained to MinContent on the INLINE axis
    // (e.g. width: min-content via CSS intrinsic sizing keywords in the inline axis only).
    ...
    } else if axis == AbstractAxis::Inline && available_grid_space.get(axis) == AvailableSpace::MinContent {
        AvailableSpace::MinContent
    } else {
        // For auto-sized containers, tracks should expand based on content,
    ...
    maximise_tracks(axis_tracks, inner_node_size.get(axis), axis_available_space_for_expansion);
