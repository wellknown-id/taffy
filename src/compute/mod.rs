...
pub fn round_layout(tree: &mut impl RoundTree, node_id: NodeId) {
    #[cfg(target_arch = "x86_64")]
    {
        if std::arch::is_x86_feature_detected!("sse4.1") {
            unsafe { round_layout_inner_sse41(tree, node_id, 0.0, 0.0) };
            return;
        }
    }
    ...
    fn round_layout_inner(tree: &mut impl RoundTree, node_id: NodeId, cumulative_x: f32, cumulative_y: f32) {
        ...
        let cx = cumulative_x + unrounded_layout.location.x;
        let cy = cumulative_y + unrounded_layout.location.y;

        let cx_r = round(cx);
        let cy_r = round(cy);
        let cxw_r = round(cx + unrounded_layout.size.width);
        let cyh_r = round(cy + unrounded_layout.size.height);

        layout.location.x = round(unrounded_layout.location.x);
        layout.location.y = round(unrounded_layout.location.y);
        layout.size.width = cxw_r - cx_r;
        layout.size.height = cyh_r - cy_r;
        ...
        layout.border.left = round(cx + unrounded_layout.border.left) - cx_r;
        layout.border.right = cxw_r - round(cx + unrounded_layout.size.width - unrounded_layout.border.right);
        layout.border.top = round(cy + unrounded_layout.border.top) - cy_r;
        layout.border.bottom = cyh_r - round(cy + unrounded_layout.size.height - unrounded_layout.border.bottom);
        layout.padding.left = round(cx + unrounded_layout.padding.left) - cx_r;
        layout.padding.right = cxw_r - round(cx + unrounded_layout.size.width - unrounded_layout.padding.right);
        layout.padding.top = round(cy + unrounded_layout.padding.top) - cy_r;
        ...
        for index in 0..child_count {
            ...
            round_layout_inner(tree, child, cx, cy);
        }
    }

    #[cfg(target_arch = "x86_64")]
    #[target_feature(enable = "sse4.1")]
    unsafe fn round_layout_inner_sse41(
        ...
        let cx_r = round_sse41(cx);
        let cy_r = round_sse41(cy);
        let cxw_r = round_sse41(cx + unrounded_layout.size.width);
        let cyh_r = round_sse41(cy + unrounded_layout.size.height);

        layout.location.x = round_sse41(unrounded_layout.location.x);
        layout.location.y = round_sse41(unrounded_layout.location.y);
        layout.size.width = cxw_r - cx_r;
        layout.size.height = cyh_r - cy_r;
        layout.scrollbar_size.width = round_sse41(unrounded_layout.scrollbar_size.width);
        layout.scrollbar_size.height = round_sse41(unrounded_layout.scrollbar_size.height);
        layout.border.left = round_sse41(cx + unrounded_layout.border.left) - cx_r;
        layout.border.right = cxw_r - round_sse41(cx + unrounded_layout.size.width - unrounded_layout.border.right);
        layout.border.top = round_sse41(cy + unrounded_layout.border.top) - cy_r;
        layout.border.bottom = cyh_r - round_sse41(cy + unrounded_layout.size.height - unrounded_layout.border.bottom);
        layout.padding.left = round_sse41(cx + unrounded_layout.padding.left) - cx_r;
        layout.padding.right = cxw_r - round_sse41(cx + unrounded_layout.size.width - unrounded_layout.padding.right);
        layout.padding.top = round_sse41(cy + unrounded_layout.padding.top) - cy_r;
        layout.padding.bottom =
            cyh_r - round_sse41(cy + unrounded_layout.size.height - unrounded_layout.padding.bottom);

        #[cfg(feature = "content_size")]
        {
            layout.content_size.width = round_sse41(cx + unrounded_layout.content_size.width) - cx_r;
            layout.content_size.height = round_sse41(cy + unrounded_layout.content_size.height) - cy_r;
        }
        ...
        for index in 0..child_count {
            let child = tree.get_child_id(node_id, index);
            round_layout_inner_sse41(tree, child, cx, cy);
        }
    }
    ...
    #[target_feature(enable = "sse4.1")]
    #[inline]
    unsafe fn round_sse41(value: f32) -> f32 {
        use std::arch::x86_64::*;
